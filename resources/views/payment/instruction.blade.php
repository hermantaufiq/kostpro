...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-3 space-y-5">
            ...
            @elseif($metode === 'uji_coba')
            {{-- ===== UJI COBA ===== --}}
            <div class="bg-white rounded-2xl shadow-soft border border-slate-100 overflow-hidden">
                <div class="bg-gradient-to-r from-slate-600 to-slate-700 px-6 py-4">
                    <p class="text-slate-200 text-xs font-medium">Metode Pembayaran</p>
                    <h2 class="text-white font-bold text-lg">Metode Uji Coba (Simulasi)</h2>
                </div>
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 bg-slate-100 text-slate-500 rounded-full flex items-center justify-center shrink-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"/></svg>
                        </div>
                        <div>
                            <p class="text-sm text-slate-600 font-medium">Ini adalah pembayaran simulasi untuk keperluan pengujian sistem.</p>
                            <p class="text-xs text-slate-500 mt-1 mb-4">Pembayaran ini tidak memproses uang sungguhan. Anda dapat menyimulasikan pembayaran lunas dengan menekan tombol di bawah ini.</p>
                            <form action="{{ route('payment.simulate', $pembayaran->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="btn-primary flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-900 text-white font-semibold text-sm transition-all shadow-md hover:shadow-lg">
